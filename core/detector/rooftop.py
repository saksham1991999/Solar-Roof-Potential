...
def contours_canny(canny_contours, edged, canny_polygons, cnts):
...
def contours_img(image_contours, edged, image_polygons, cnts):
...
def panel_rotation(new_image, l, w, pw, pl, solar_angle, panels_series, high_reso_orig, fname, solar_roof_area):
...
def detect(img_path):
    ...
    response = requests.get(
        "https://developer.nrel.gov/api/pvwatts/v6.json?api_key=fSOibWSTKTlii210ZqBoyfIAYjA3lhjQCrq4y2Vr&lat=" + lat + "&lon=" + lon + "&system_capacity=4&azimuth=180&tilt=40&array_type=1&module_type=1&losses=10")
    ...
    for fname in images:
        ...
        # print(image_contours)
        contours_img(image_contours, edged, image_polygons, cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2])
        # contours_img(cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2])
        # Contours in Canny Edge Image
        contours_canny(canny_contours, edged, canny_polygons, cv2.findContours(edged, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2])
        # contours_canny()
